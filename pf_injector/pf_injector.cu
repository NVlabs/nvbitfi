...
void nvbit_at_cuda_event(CUcontext ctx, int is_exit, nvbit_api_cuda_t cbid,
  ...
  if (cbid == API_CUDA_cuLaunch ||
      ...
      if(!is_exit) {
          ...
          nvbit_enable_instrumented(ctx, p->f, true); // we always want to run the instrumented run (for now)
          cudaDeviceSynchronize();
      }  else {
          ...
          fout << "ctas: " << num_ctas << std::endl;
				  fout << "selected SM: " << inj_info.injSMID << std::endl;
				  fout << "selected Lane: " << inj_info.injLaneID << std::endl;
				  fout << "selected Mask: " << inj_info.injMask << std::endl;
				  fout << "selected InstType: " << inj_info.injInstType << std::endl;
